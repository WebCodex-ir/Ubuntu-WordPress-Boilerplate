...
بسیار خب. لاگ شما نشان می‌دهد که سرور شما از قبل یک فایل swap داشته است، اما حجم آن فقط ۵۱۲ مگابایت بوده که کافی نیست. به همین دلیل دستورات با خطا مواجه شدند.

راه حل: افزایش حجم فایل Swap
ما فایل swap قدیمی و کوچک را خاموش کرده، آن را حذف می‌کنیم و سپس یک فایل جدید و بزرگتر ۲ گیگابایتی می‌سازیم.

این دستورات را به ترتیب و یکی یکی اجرا کنید:

خاموش کردن فایل swap قدیمی:
...
sudo rm /swapfile
```
ایجاد فایل swap جدید با حجم ۲ گیگابایت:
...
```
آماده‌سازی فایل جدید:

```bash
sudo mkswap /swapfile
```
فعال‌سازی فایل swap جدید:
...
```
تایید نهایی:
برای اطمینان از اینکه فایل جدید و بزرگتر فعال شده، این دستور را اجرا کنید:

```bash

sudo swapon --show
```
این بار در خروجی، زیر ستون SIZE باید حجم 2G را ببینید.

بعد از انجام این مراحل، مشکل کمبود حافظه سرور شما برای همیشه حل شده و می‌توانید به راحتی بکاپ بگیرید.
